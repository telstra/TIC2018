#include "mqttpackets.h"
  ...
  MQTT_Packets::MQTT_Packets(TelstraM1Interface *shield)
  {
  	this->shield = shield;
  }
...
int16_t MQTT_Packets::Connect(char *hostname, uint16_t port, mqtt_client_info *client_info, mqtt_message *last_will_msg) {
  ...
  /* send the packet */
  shield->sendCommand(this->commandBuffer, c_size);

  Serial.println("Command sent, waiting for response");

  /* wait for response and get response */
  this->time_out = shield->getResponse(this->commandBuffer, BUFFER_SIZE, MAXTIMEOUT);
...
int16_t MQTT_Packets::Publish(mqtt_message *publish_msg) {
  ...
  /* send the packet */
  //shield->sendCommand("TestingPublish", c_size);
  shield->sendCommand(this->commandBuffer, c_size);
...
int16_t MQTT_Packets::GetMessage(char *topicDest, char *contentDest, int timeout) {      //when time out or no message avalable this will return 0
	...
	if(shield->MessageAvailable()) {
    bytesRead = shield->getResponse(this->commandBuffer, BUFFER_SIZE, timeout);
